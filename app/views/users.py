...
def get_users():
    ...
    if users:
        ...
        return jsonify({'message': 'sussefully fetched', 'data': result.data})
...
def get_user(id):
    ...
    if user:
        result = user_schema.dump(user)
        return jsonify({'message': 'sussefully fetched', 'data': result.data}), 201

    return jsonify({'message': 'user do not exist', 'data': {}}), 404
